...
// In your CandidatureRepository
public function findStatisticsBySpecialityAndExperience(): array
{
    return $this->createQueryBuilder('c')
        ->select('c.specialite, c.nbranneexper AS experience, COUNT(c.id) AS count')
        ->groupBy('c.specialite, c.nbranneexper')
        ->getQuery() // Convert QueryBuilder to Query
        ->getArrayResult(); // Now call getArrayResult() on the Query object
}
